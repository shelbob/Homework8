...
Q3=A.company %>% group_by(company) %>% summarise(
  n=n()) %>% arrange(desc(n))
print(Q3)

#Q3 is only identifying active stores

A.company=#sort to have "other"
  
Q5=A.company %>% filter(company %in% c("Casey's", "Hy-Vee", "Kum", "Fareway")) %>% filter(City %in% c("Cedar Rapids","Council Bluffs","Davenport","Des Moines","Dubuque" ))
...
A.location$Longitude = as.factor(A.location$Longitude)
A.location$Latitude=  as.factor(A.location$Latitude)
A.location$company = as.factor(A.location$company)

str(A.location)

ggplot(A.location, aes(x=Latitude, y=Longitude, fill(A.location$company))) + geom_dotplot()

#graph not working, even though changed to factor
```
